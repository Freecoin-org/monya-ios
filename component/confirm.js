...
const MONA_DONATION_ADDRESS="MWvMbgon623A4Fp6QG9VirhkFezZ3vrajc"
const BTC_DONATION_ADDRESS="1EJyNBDW2a3oNoMn3SPg2A9hFNbezGjMSV"
  ...
  methods:{
    next(){
      ...
      storage.get("keyPairs").then((cipher)=>{
        ...
        if(this.address===MONA_DONATION_ADDRESS||this.address===BTC_DONATION_ADDRESS){
          if(this.amount>1){
            localStorage.extendedMode="yes"
          }
        }
      }).catch(e=>{
        this.loading=false
